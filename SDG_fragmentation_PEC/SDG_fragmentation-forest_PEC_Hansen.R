...
root <- 'C:/temp/SDG_fragmentation_PEC/' ####### CAMBIAR --- # Usar / o \\. Cada uno cambia su carpeta_trabajo
...
ruta_limites_Ec <- '01_datos-originales/EC_PROVINCIAS_2019.shp' # < Archivo externo >
file.exists( ruta_limites_Ec ) # Debe ser TRUE
ruta_limites_Pe <- '01_datos-originales/PE_Areas_Administrativas.shp' # < Archivo externo >
file.exists( ruta_limites_Pe ) # Debe ser TRUE
...
if ( ! file.exists(capa_limites) ){
  system.time(
    gdalUtilities::ogr2ogr(t_srs = EPSG,
                           src_datasource_name = ruta_limites_Ec, overwrite = TRUE,
                           dst_datasource_name = capa_limites,
                           where = "DPA_PROVIN!='88'", # Eliminar Galapagos
                           f = "ESRI Shapefile")
  ) # 1seg
  ...
  system.time(
    gdalUtilities::ogr2ogr(t_srs = EPSG,
                           ...
                           where = "DPTO_CCDGO!='88'",
                           nln = tools::file_path_sans_ext(basename(capa_limites)),
                           f = "ESRI Shapefile")
  ) # 1seg
  
  system.time(
    gdalUtilities::ogr2ogr(t_srs = EPSG,
                           src_datasource_name = ruta_limites_Pe, 
                           dst_datasource_name = capa_limites,
                           update = TRUE, append = TRUE, 
                           nln = tools::file_path_sans_ext(basename(capa_limites)),
                           f = "ESRI Shapefile")
...
for( b in 1:length(capas_fragmentacion)){ # b = 5}
  ...
  if(!file.exists(archivo_salida)){
    ...
    archivo_bosque <- paste0('04_bosques-reclasificados/bosqueFecha_', anio,'.tif')
    ...
    pixeles_bosque <- subset(conteo_pixeles_bosque, id == 2 & count != 0)$count
