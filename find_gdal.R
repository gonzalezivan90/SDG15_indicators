gdalPaths <- function(depth = 3, drives = c('C'), help = FALSE){
  if (help){
  ...
  if (length(drives) == 0){
    ...
    for(i in 1:length(drives)){ # i = 1
      ...
      for(j in 1:depth){ # j = 1
        folders_level <- new_folders <-  unique(c( paste0(drives[i],':/', c('Program Files', '.')), 
                            gsub('//', '/', list.dirs(drive, recursive = FALSE) )) )
        
        if(j >= 2 ){ # j = 2
          temp_folder <- folders_level
          for(n in 2:depth){
            new_folders <- unname(unlist(sapply(temp_folder, list.dirs, recursive = FALSE)))
            temp_folder <- c(temp_folder, new_folders)
          }
          folders_level <- new_folders
        }
        # print(tail(temp_folder))
      
        
        for (k in 1:length(folders_level)){ # k = 1
          folder <- folders_level[k]
          (try1 <- dir.exists( folder ))
          if (try1){
            dirs1 <- list.dirs(path = folder, recursive = FALSE)
            qgisFolders <- grep('QGIS', toupper( dirs1 ))
            if (length(qgisFolders) >= 1){
              (qgis <- dirs1[tail(qgisFolders, 1)])
              OSGeo4W <- paste0(qgis, '/OSGeo4W.bat')
              arg1_exists <- file.exists(OSGeo4W)
              if(arg1_exists) {
                arg1 <- OSGeo4W
                o4w_env <- paste0(qgis, '/bin/o4w_env.bat')
                arg2_exists <- file.exists(o4w_env)
                if (arg2_exists){
                  arg2 <- o4w_env
                } else {
                  py3_env <- paste0(qgis, '/py3_env.bat')
                ...
                if ( arg1_exists & arg2_exists){
                  print(paste(drive, folder, arg1, arg2))
                  found <<- TRUE
                  ...
                  (gdalcalc <- (system(paste0(execGDALcalc, ' --help'), intern = TRUE)))
                  ...
                  ans <- list(execGDALcalc, gdalcalc, execGDALsieve, gdalsieve, execGDALproxy, gdalproxy)
                  break()
                }
              }
            }
...
gdal <- gdalPaths(depth = 3, drives = c('C'), help = FALSE)
